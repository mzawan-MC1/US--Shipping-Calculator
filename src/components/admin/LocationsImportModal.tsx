...
import { AdminPurchaseLocation, AdminState } from '../../services/adminService';
import {
...
interface LocationsImportModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const LocationsImportModal: React.FC<LocationsImportModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  existingLocations,
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await locationsBulkService.parseFile(file, existingLocations, states);
      setSummary(res);
    } catch (err: unknown) {
  ...
  const handleCommit = async () => {
    ...
    try {
      const { appliedCount } = await locationsBulkService.commitImport(summary.rows);
      onSuccess(`Successfully imported/updated ${appliedCount} pickup location(s).`);
      onClose();
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to commit imported locations.');
    } finally {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Bulk Import Pickup Locations (CSV / Excel)"
    >
        ...
        <div className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-200">
          ...
          <div className="flex items-center gap-1.5">
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => locationsBulkService.downloadTemplate('csv')}
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => locationsBulkService.downloadTemplate('xlsx')}
              className="text-[11px] font-bold"
            >
              ...
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl flex items-center justify-between text-rose-900">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => locationsBulkService.downloadErrorReport(rejectedRows, 'csv')}
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => locationsBulkService.downloadErrorReport(rejectedRows, 'xlsx')}
                    className="text-[10px] border-rose-300 text-rose-800 hover:bg-rose-100"
                  >
                    Excel Report
                  </Button>
        ...
        <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
          ...
          {summary && importableCount > 0 && (
            <Button
              ...
              className="font-bold"
            >
              {isCommitting ? 'Importing Locations...' : `Confirm & Import (${importableCount} Locations)`}
            </Button>
          )}
        </div>
      </div>
