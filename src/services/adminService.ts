...
export const adminService = {
  ...
  async getQuotations(): Promise<AdminQuotation[]> {
    ...
    try {
      ...
      if (error || !data) {
        console.warn('[AdminService] Quotations load note:', error?.message);
        return [];
      }
  ...
  async createCustomer(customer: {
    ...
    if (isDemoMode) return { success: true, id: `demo-cust-${Date.now()}` };
  ...
  async getExchangeRate(): Promise<{ rate: number; updatedAt: string }> {
    if (isDemoMode) return { rate: 3.6725, updatedAt: new Date().toISOString() };
    try {
      ...
      return { rate: 3.6725, updatedAt: new Date().toISOString() };
    }
  },
  ...
  async getAuditEvents(): Promise<AdminAuditEvent[]> {
    ...
    try {
      ...
      return rows.map((a) => ({
        ...
        performedByName: a.staff_profiles?.full_name || 'Automated Engine',
