-- ==============================================================================
-- Migration: 20260919010000_simplify_vehicle_attributes_pricing.sql
-- Description:
-- 1. Add extra_towing_charge, extra_shipping_charge, charge_reason_en, charge_reason_ar
--    to powertrains and vehicle_conditions.
-- 2. Migrate existing active adjustments into the respective attribute master tables.
-- 3. Create synchronization triggers on powertrains and vehicle_conditions.
-- 4. Update calculate_shipping_quote_v1 to show clear attribute names and saved reasons.
-- ==============================================================================

-- 1. EXTEND POWERTRAINS AND VEHICLE_CONDITIONS TABLES
ALTER TABLE public.powertrains
    ADD COLUMN IF NOT EXISTS extra_towing_charge NUMERIC(12, 2) NOT NULL DEFAULT 0.00 CHECK (extra_towing_charge >= 0),
    ADD COLUMN IF NOT EXISTS extra_shipping_charge NUMERIC(12, 2) NOT NULL DEFAULT 0.00 CHECK (extra_shipping_charge >= 0),
    ADD COLUMN IF NOT EXISTS charge_reason_en TEXT,
    ADD COLUMN IF NOT EXISTS charge_reason_ar TEXT;
...
-- 2. MIGRATE EXISTING PRICE ADJUSTMENTS INTO MASTER ATTRIBUTE TABLES
-- Powertrain Towing Adjustments
UPDATE public.powertrains p
SET extra_towing_charge = a.amount_usd,
    charge_reason_en = COALESCE(NULLIF(TRIM(p.charge_reason_en), ''), a.reason_en),
    charge_reason_ar = COALESCE(NULLIF(TRIM(p.charge_reason_ar), ''), a.reason_ar)
FROM public.attribute_price_adjustments a
WHERE a.attribute_type = 'powertrain'
  AND a.attribute_id = p.id
  AND a.context = 'towing'
  AND a.is_active = true;

-- Powertrain Shipping Adjustments
UPDATE public.powertrains p
SET extra_shipping_charge = a.amount_usd,
    charge_reason_en = COALESCE(NULLIF(TRIM(p.charge_reason_en), ''), a.reason_en),
    charge_reason_ar = COALESCE(NULLIF(TRIM(p.charge_reason_ar), ''), a.reason_ar)
FROM public.attribute_price_adjustments a
WHERE a.attribute_type = 'powertrain'
  AND a.attribute_id = p.id
  AND a.context = 'shipping'
  AND a.is_active = true;

-- Vehicle Condition Towing Adjustments
UPDATE public.vehicle_conditions c
SET extra_towing_charge = a.amount_usd,
    charge_reason_en = COALESCE(NULLIF(TRIM(c.charge_reason_en), ''), a.reason_en),
    charge_reason_ar = COALESCE(NULLIF(TRIM(c.charge_reason_ar), ''), a.reason_ar)
FROM public.attribute_price_adjustments a
WHERE a.attribute_type = 'vehicle_condition'
  AND a.attribute_id = c.id
  AND a.context = 'towing'
  AND a.is_active = true;

-- Vehicle Condition Shipping Adjustments
UPDATE public.vehicle_conditions c
SET extra_shipping_charge = a.amount_usd,
    charge_reason_en = COALESCE(NULLIF(TRIM(c.charge_reason_en), ''), a.reason_en),
    charge_reason_ar = COALESCE(NULLIF(TRIM(c.charge_reason_ar), ''), a.reason_ar)
...
-- Vehicle Category Towing Adjustments
UPDATE public.vehicle_categories vc
SET extra_towing_charge = a.amount_usd,
    charge_reason_en = COALESCE(NULLIF(TRIM(vc.charge_reason_en), ''), a.reason_en),
    charge_reason_ar = COALESCE(NULLIF(TRIM(vc.charge_reason_ar), ''), a.reason_ar)
FROM public.attribute_price_adjustments a
WHERE a.attribute_type = 'vehicle_category'
  AND a.attribute_id = vc.id
  AND a.context = 'towing'
  AND a.is_active = true
  AND vc.extra_towing_charge = 0;

-- Vehicle Category Shipping Adjustments
UPDATE public.vehicle_categories vc
SET extra_shipping_charge = a.amount_usd,
    charge_reason_en = COALESCE(NULLIF(TRIM(vc.charge_reason_en), ''), a.reason_en),
    charge_reason_ar = COALESCE(NULLIF(TRIM(vc.charge_reason_ar), ''), a.reason_ar)
...
WHERE a.attribute_type = 'vehicle_category'
  ...
  AND a.context = 'shipping'
  AND a.is_active = true
  AND vc.extra_shipping_charge = 0;

-- 3. TRIGGER FUNCTIONS TO MAINTAIN ATTRIBUTE_PRICE_ADJUSTMENTS SYNCHRONIZED
-- Powertrains trigger
CREATE OR REPLACE FUNCTION public.sync_powertrain_adjustments()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    IF TG_OP = 'DELETE' THEN
        DELETE FROM public.attribute_price_adjustments
        WHERE attribute_type = 'powertrain' AND attribute_id = OLD.id;
        RETURN OLD;
    END IF;

    -- Towing
    DELETE FROM public.attribute_price_adjustments
    WHERE attribute_type = 'powertrain'
      AND attribute_id = NEW.id
      AND context = 'towing';

    IF NEW.extra_towing_charge > 0 THEN
        INSERT INTO public.attribute_price_adjustments (
            ...
            'powertrain', NEW.id, 'towing', NEW.extra_towing_charge,
            COALESCE(NULLIF(TRIM(NEW.charge_reason_en), ''), NEW.name || ' Handling'),
            NULLIF(TRIM(NEW.charge_reason_ar), ''),
            (NEW.is_active AND NOT COALESCE(NEW.is_archived, false)),
    ...
    IF NEW.extra_shipping_charge > 0 THEN
        INSERT INTO public.attribute_price_adjustments (
            attribute_type, attribute_id, context, amount_usd, reason_en, reason_ar, is_active, display_order
        ) VALUES (
            'powertrain', NEW.id, 'shipping', NEW.extra_shipping_charge,
            COALESCE(NULLIF(TRIM(NEW.charge_reason_en), ''), NEW.name || ' Handling'),
            NULLIF(TRIM(NEW.charge_reason_ar), ''),
            (NEW.is_active AND NOT COALESCE(NEW.is_archived, false)),
            COALESCE(NEW.display_order, 0)
...
DROP TRIGGER IF EXISTS trg_sync_powertrain_adjustments ON public.powertrains;
CREATE TRIGGER trg_sync_powertrain_adjustments
    AFTER INSERT OR UPDATE OR DELETE ON public.powertrains
    FOR EACH ROW
    EXECUTE FUNCTION public.sync_powertrain_adjustments();

-- Vehicle Conditions trigger
CREATE OR REPLACE FUNCTION public.sync_vehicle_condition_adjustments()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public', 'pg_temp'
...
BEGIN
    IF TG_OP = 'DELETE' THEN
        DELETE FROM public.attribute_price_adjustments
        WHERE attribute_type = 'vehicle_condition' AND attribute_id = OLD.id;
        RETURN OLD;
    END IF;

    -- Towing
    DELETE FROM public.attribute_price_adjustments
    WHERE attribute_type = 'vehicle_condition'
      AND attribute_id = NEW.id
      AND context = 'towing';
    ...
    IF NEW.extra_towing_charge > 0 THEN
        INSERT INTO public.attribute_price_adjustments (
            ...
            'vehicle_condition', NEW.id, 'towing', NEW.extra_towing_charge,
            COALESCE(NULLIF(TRIM(NEW.charge_reason_en), ''), NEW.name || ' Handling'),
            NULLIF(TRIM(NEW.charge_reason_ar), ''),
            (NEW.is_active AND NOT COALESCE(NEW.is_archived, false)),
            COALESCE(NEW.display_order, 0)
    ...
    IF NEW.extra_shipping_charge > 0 THEN
        INSERT INTO public.attribute_price_adjustments (
            ...
            'vehicle_condition', NEW.id, 'shipping', NEW.extra_shipping_charge,
            COALESCE(NULLIF(TRIM(NEW.charge_reason_en), ''), NEW.name || ' Handling'),
            NULLIF(TRIM(NEW.charge_reason_ar), ''),
            (NEW.is_active AND NOT COALESCE(NEW.is_archived, false)),
            COALESCE(NEW.display_order, 0)
...
DROP TRIGGER IF EXISTS trg_sync_vehicle_condition_adjustments ON public.vehicle_conditions;
CREATE TRIGGER trg_sync_vehicle_condition_adjustments
    AFTER INSERT OR UPDATE OR DELETE ON public.vehicle_conditions
    FOR EACH ROW
    EXECUTE FUNCTION public.sync_vehicle_condition_adjustments();

-- 4. UPDATE CALCULATE_SHIPPING_QUOTE_V1 FOR CRYSTAL-CLEAR ATTRIBUTE LINE ITEMS
...
BEGIN
    ...
    LOOP
        v_line_items := v_line_items || jsonb_build_object(
            ...
            'description', CASE 
                WHEN v_adj.type = 'vehicle_category' THEN
                    'Vehicle Category Adjustment — ' || v_category_name || CASE 
                        WHEN v_adj.reason_en IS NOT NULL 
                          AND TRIM(v_adj.reason_en) != '' 
                          AND TRIM(v_adj.reason_en) != v_category_name 
                          AND TRIM(v_adj.reason_en) != (v_category_name || ' Handling') 
                        THEN ' (' || TRIM(v_adj.reason_en) || ')' 
                        ELSE '' 
                    END
                WHEN v_adj.type = 'powertrain' THEN
                    'Powertrain Adjustment — ' || v_powertrain_name || CASE 
                        WHEN v_adj.reason_en IS NOT NULL 
                          AND TRIM(v_adj.reason_en) != '' 
                          AND TRIM(v_adj.reason_en) != v_powertrain_name 
                          AND TRIM(v_adj.reason_en) != (v_powertrain_name || ' Handling') 
                        THEN ' (' || TRIM(v_adj.reason_en) || ')' 
                        ELSE '' 
                    END
                WHEN v_adj.type = 'vehicle_condition' THEN
                    'Vehicle Condition Adjustment — ' || v_condition_name || CASE 
                        WHEN v_adj.reason_en IS NOT NULL 
                          AND TRIM(v_adj.reason_en) != '' 
                          AND TRIM(v_adj.reason_en) != v_condition_name 
                          AND TRIM(v_adj.reason_en) != (v_condition_name || ' Handling') 
                        THEN ' (' || TRIM(v_adj.reason_en) || ')' 
                        ELSE '' 
                    END
                ELSE 'Shipping Adjustment: ' || v_adj.reason_en
            END,
            'description_ar', CASE 
                WHEN v_adj.type = 'vehicle_category' THEN
                    'تعديل فئة المركبة — ' || COALESCE(v_category.name_ar, v_category_name) || CASE 
                        WHEN v_adj.reason_ar IS NOT NULL AND TRIM(v_adj.reason_ar) != '' 
                        THEN ' (' || TRIM(v_adj.reason_ar) || ')' 
                        ELSE '' 
                    END
                WHEN v_adj.type = 'powertrain' THEN
                    'تعديل نوع المحرك — ' || COALESCE(v_powertrain.name_ar, v_powertrain_name) || CASE 
                        WHEN v_adj.reason_ar IS NOT NULL AND TRIM(v_adj.reason_ar) != '' 
                        THEN ' (' || TRIM(v_adj.reason_ar) || ')' 
                        ELSE '' 
                    END
                WHEN v_adj.type = 'vehicle_condition' THEN
                    'تعديل الحالة التشغيلية — ' || COALESCE(v_condition.name_ar, v_condition_name) || CASE 
                        WHEN v_adj.reason_ar IS NOT NULL AND TRIM(v_adj.reason_ar) != '' 
                        THEN ' (' || TRIM(v_adj.reason_ar) || ')' 
                        ELSE '' 
                    END
                ELSE COALESCE(v_adj.reason_ar, v_adj.reason_en)
            END,
            'amount_usd', v_adj.amount_usd
        );
    END LOOP;
            ...
            'description', CASE 
                ...
                WHEN v_adj.type = 'vehicle_condition' THEN
                    'Vehicle Condition Adjustment — ' || v_condition_name || CASE 
                        ...
                        ELSE '' 
                    END
                ELSE 'Towing Adjustment: ' || v_adj.reason_en
            END,
            'description_ar', CASE 
                WHEN v_adj.type = 'vehicle_category' THEN
                    'تعديل فئة المركبة — ' || COALESCE(v_category.name_ar, v_category_name) || CASE 
