...
-- Description: Hardening contact enquiry RPC, concurrency sequence, abuse prevention, and explicit privilege boundaries
...
-- 3. Replace submit_contact_enquiry with hardened input validation, rate limiting, and safe public return payload (no UUIDs)
...
BEGIN
    ...
    v_subject     := trim(coalesce(input_json->>'subject', ''));
    ...
    -- 6. Subject validation against approved enum keys
    IF v_subject NOT IN (
...
-- 4. Apply exact execution privileges
