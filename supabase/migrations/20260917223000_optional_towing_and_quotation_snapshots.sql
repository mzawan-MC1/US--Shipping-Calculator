-- ==============================================================================
-- Migration: 20260917223000_optional_towing_and_quotation_snapshots.sql
-- Description:
-- 1. Support optional inland towing with include_inland_towing boolean
-- 2. Store rich human-readable names and codes in immutable quotation snapshots (no raw UUIDs)
-- 3. Correct line items and financial subtotal structures
-- ==============================================================================
...
BEGIN
    ...
    -- Parse optional inland towing flag
    ...
    IF v_include_inland_towing AND (input_json->>'purchase_location_id') IS NOT NULL AND TRIM(input_json->>'purchase_location_id') != '' THEN
        BEGIN
    ...
    -- Inland Towing Resolution
    IF NOT v_include_inland_towing THEN
        v_towing_min := 0.00;
        v_towing_max := 0.00;
    ...
    ELSE
        IF v_purchase_location_id IS NOT NULL THEN
            ...
            WHERE id = v_purchase_location_id;

            IF FOUND THEN
                v_purchase_location_name := v_purchase_location.name;
                v_purchase_location_city := v_purchase_location.name;
            ...
            IF FOUND THEN
                IF v_towing_record.rate_type = 'fixed' THEN
                    ...
                    v_is_towing_range := false;
                    v_towing_description := 'Inland Towing (' || COALESCE(v_purchase_location_name, 'Pickup') || ' to ' || v_origin_port.name || ')';
                ELSE
                    v_towing_min := v_towing_record.min_amount;
                    v_towing_max := v_towing_record.max_amount;
                    v_is_towing_range := true;
                    v_towing_description := 'Inland Towing (' || COALESCE(v_purchase_location_name, 'Pickup') || ' to ' || v_origin_port.name || ' - Estimated range)';
                END IF;
            ELSE
                v_towing_min := 350.00;
                v_towing_max := 550.00;
                v_is_towing_range := true;
                v_towing_description := 'Inland Towing (' || COALESCE(v_purchase_location_name, 'Pickup') || ' to ' || v_origin_port.name || ' - Standard estimate)';
            END IF;
        ELSE
            -- Towing is requested without specific bracket
            v_towing_min := 350.00;
            v_towing_max := 550.00;
            v_is_towing_range := true;
            v_towing_description := 'Inland Towing (US Auction to ' || v_origin_port.name || ' - Standard estimate)';
        END IF;
    END IF;
    ...
    -- Total shipping & clearance charges payable (Strictly excludes declared vehicle purchase price):
    ...
    -- 10. Financial Line Items (Strictly Ordered)
    v_line_items := jsonb_build_array(
        jsonb_build_object(
            'category', 'ocean_freight',
            'description', 'Ocean Freight Tariff (' || v_origin_port.name || ' to ' || v_dest_port.name || ')',
    ...
    -- 10b. Active Quotation Rules & Regulations Snapshot
    SELECT COALESCE(
        jsonb_agg(
            jsonb_build_object(
                'id', r.id,
    ...
    -- 11. Comprehensive Human-Readable Pricing Snapshot (No Raw UUIDs)
