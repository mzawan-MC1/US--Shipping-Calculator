-- ==============================================================================
-- Migration: 20260919003000_vehicle_category_adjustments.sql
-- Description:
-- 1. Add extra_towing_charge, extra_shipping_charge, charge_reason_en, charge_reason_ar
--    to public.vehicle_categories.
-- 2. Create trigger sync_vehicle_category_adjustments to automatically sync
--    extra charges with public.attribute_price_adjustments.
-- 3. Update calculate_shipping_quote_v1 to:
--    - Apply vehicle_category adjustments under towing (when inland towing is requested)
--    - Format category adjustments as "Vehicle Category Adjustment — <Category Name>" (+ reason)
--    - Retain all CIF, statutory customs duty (5%), and VAT (5%) logic unchanged.
-- ==============================================================================

-- 1. EXTEND VEHICLE CATEGORIES TABLE
...
-- 2. TRIGGER TO KEEP ATTRIBUTE_PRICE_ADJUSTMENTS SYNCHRONIZED
CREATE OR REPLACE FUNCTION public.sync_vehicle_category_adjustments()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
    -- Handle deletion of vehicle category
    IF TG_OP = 'DELETE' THEN
        DELETE FROM public.attribute_price_adjustments
        WHERE attribute_type = 'vehicle_category' AND attribute_id = OLD.id;
        RETURN OLD;
    END IF;

    -- Synchronize Towing Adjustment
    DELETE FROM public.attribute_price_adjustments
    WHERE attribute_type = 'vehicle_category'
    ...
    -- Synchronize Shipping Adjustment
    DELETE FROM public.attribute_price_adjustments
    WHERE attribute_type = 'vehicle_category'
      AND attribute_id = NEW.id
...
DROP TRIGGER IF EXISTS trg_sync_vehicle_category_adjustments ON public.vehicle_categories;
CREATE TRIGGER trg_sync_vehicle_category_adjustments
    AFTER INSERT OR UPDATE OR DELETE ON public.vehicle_categories
    FOR EACH ROW
    EXECUTE FUNCTION public.sync_vehicle_category_adjustments();

-- 3. UPDATE CALCULATE_SHIPPING_QUOTE_V1
