-- =============================================================================
-- Migration: 20260918210000_simplify_towing_rates.sql
-- Description: Simplify inland towing rates to strictly Location-to-Port
--              relationships. Vehicle category no longer impacts towing rate
--              lookup, availability, or pricing calculations.
-- =============================================================================

-- 1. Consolidate existing towing rates (remove category & condition dependency)
UPDATE public.towing_rates
SET vehicle_category_id = NULL,
    vehicle_condition_id = NULL,
    updated_at = now();

-- 2. Drop legacy category-dependent unique index and enforce unique active rate per (location, port)
DROP INDEX IF EXISTS public.uq_active_towing_rate;

CREATE UNIQUE INDEX uq_active_towing_rate
ON public.towing_rates (purchase_location_id, loading_port_id)
WHERE (is_active = true AND effective_to IS NULL);

-- 3. Update preview_bulk_towing_adjustment RPC
...
-- 4. Update apply_bulk_towing_adjustment RPC
...
BEGIN
    ...
    LOOP
        ...
        IF v_effective_from > CURRENT_DATE THEN
            -- Expire current rate day before new rate takes effect
            UPDATE public.towing_rates
            SET effective_to = (v_effective_from - INTERVAL '1 day')::DATE, updated_at = now()
            WHERE id = v_rec.id;

            -- Insert new rate version without category
            INSERT INTO public.towing_rates (
                ...
                v_rec.purchase_location_id, v_rec.loading_port_id, NULL, NULL,
...
-- 5. Update get_calculator_availability_v1 to remove category filtering on towing
...
BEGIN
    ...
    SELECT COALESCE(
        jsonb_agg(
            jsonb_build_object(
                ...
                'description_ar', sm.description_ar,
                'is_container', sm.is_container,
    ...
    WHERE sm.is_active = true AND sm.is_archived = false
      AND (
...
-- 6. Update calculate_shipping_quote_v1 to strictly use location-to-port towing rate
