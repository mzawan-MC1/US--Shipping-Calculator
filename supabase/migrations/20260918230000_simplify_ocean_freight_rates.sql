-- =============================================================================
-- Migration: 20260918230000_simplify_ocean_freight_rates.sql
-- Description: Simplify Ocean Freight Tariffs to strictly Route + Shipping Method.
--              Vehicle category and powertrain no longer impact ocean freight
--              rate lookup, availability, or pricing calculations.
-- =============================================================================

-- 1. Drop NOT NULL constraints on vehicle_category_id and powertrain_id
ALTER TABLE public.route_freight_rates ALTER COLUMN vehicle_category_id DROP NOT NULL;
ALTER TABLE public.route_freight_rates ALTER COLUMN powertrain_id DROP NOT NULL;

-- 2. Consolidate existing records per (route_id, shipping_method_id)
DO 
DECLARE
    r RECORD;
    v_kept_id UUID;
BEGIN
    FOR r IN
        SELECT DISTINCT route_id, shipping_method_id
        FROM public.route_freight_rates
        WHERE is_active = true
    LOOP
        -- Find newest active record to keep
        SELECT id INTO v_kept_id
        FROM public.route_freight_rates
        WHERE route_id = r.route_id
          AND shipping_method_id = r.shipping_method_id
          AND is_active = true
        ORDER BY created_at DESC, id DESC
        LIMIT 1;

        -- Keep newest active and clear vehicle category / powertrain
        UPDATE public.route_freight_rates
        SET vehicle_category_id = NULL,
            powertrain_id = NULL,
            updated_at = now()
        WHERE id = v_kept_id;

        -- Deactivate older duplicate active records
        UPDATE public.route_freight_rates
        SET is_active = false,
...
-- 3. Replace index: enforce unique active rate per (route_id, shipping_method_id)
DROP INDEX IF EXISTS public.uq_active_route_freight_rate;
DROP INDEX IF EXISTS public.idx_freight_lookup;

CREATE UNIQUE INDEX uq_active_route_freight_rate
ON public.route_freight_rates (route_id, shipping_method_id)
WHERE is_active = true AND effective_to IS NULL;

CREATE INDEX idx_freight_lookup
ON public.route_freight_rates(route_id, shipping_method_id)
WHERE is_active = true;

-- 4. Update get_calculator_availability_v1
...
BEGIN
    ...
    -- Universal across vehicle categories
    SELECT COALESCE(
        jsonb_agg(
            jsonb_build_object(
                'code', st.code,
    ...
    -- 3. Eligible Pickup Locations (Universal across vehicle categories)
    SELECT COALESCE(
        jsonb_agg(
            jsonb_build_object(
                'id', loc.id,
    ...
    -- 6. Eligible Shipping Methods (Only active methods with an active rate on the route)
    SELECT COALESCE(
        jsonb_agg(
            jsonb_build_object(
                'id', sm.id,
    ...
    WHERE sm.is_active = true
      AND (
        v_origin_port_id IS NULL OR v_destination_port_id IS NULL
        OR EXISTS (
            SELECT 1 FROM public.shipping_routes sr
...
-- 5. Update calculate_shipping_quote_v1
    ...
    IF v_include_inland_towing THEN
        FOR v_adj IN
            ...
            WHERE is_active = true
              ...
              AND (effective_to IS NULL OR effective_to >= CURRENT_DATE)
              AND (
                (attribute_type = 'powertrain' AND attribute_id = v_powertrain_id)
